...
TOKEN = '7492841982:AAE-3qJSC-OQfKzF0e3AHHL7I4-R3SvwzSY'
...
def attack_command(message):
    ...
    try:
        ...
        if not user_data or user_data['plan'] == 0:
            bot.send_message(chat_id, "You are not approved to use this bot. Please contact the administrator 👉 @tnzspy .")
            return
        ...
        bot.send_message(chat_id, "✅IP =\ {port} <seconds>>✓..")
...
def attack_command(message):
    ...
    try:
        ...
        bot.send_message(chat_id, "*✅IP =\ {port} <seconds>✓.*", parse_mode='Markdown')
        bot.register_next_step_handler(message, process_attack_command)
    except Exception as e:
        logging.error(f"Error in attack command: {e}")
...
def process_attack_command(message):
    try:
        ...
        if len(args) != 3:
            bot.send_message(message.chat.id, "*✅FEEDBACK SEND ✓ /Attack*", parse_mode='Markdown')
            return
        ...
        bot.send_message(message.chat.id, f"*Attack started✓\nᏢᴀʏᴀʟ🦋⃟ DODS✓\n\nIP: {target_ip}\nPort: {target_port}\nSeconds: {duration}*", parse_mode='Markdown')
    except Exception as e:
...
def handle_message(message):
    if message.text == "Attack ":
        bot.reply_to(message, "*✅ VIP = SERVER  USER ✓*", parse_mode='Markdown')
    elif message.text == "Attack 🚀":
        bot.reply_to(message, "*✅ VIP = SERVER  USER ✓*", parse_mode='Markdown')
        attack_command(message)
    elif message.text == "Plan✔️":
        bot.send_message(message.chat.id, "*💞 Ꮲᴀʏᴀʟ🦋⃟ DODS 💞\n\n🎮 TRIAL ONLY 1 DAY\n🎁OFFER 3.4\n💞 DAY ☞ ₹ 59\n💞WEEK ☞ ₹ 299\n💞MONTH ☞ ₹ 599\n💞SEASON ☞ ₹ 1299\n\n✅DM TO BUY VIP \nSERVER HACK 3.4💘\nDM ☞ @payalyt70.*", parse_mode='Markdown')
